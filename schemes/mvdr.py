...
class MVDR(BaseBaseline): 
    """
    讓 MVDR 專案可以被 main.py 以統一流程呼叫：
    ...
    def setup(self, args) -> None:
        """
        1) 檢查 MVDR 原始碼是否存在於 schemes/GR-as-MVDR
        2) 設定工作路徑、環境變數
        3) 做資料格式轉換
        """
        ...
        # 將共用 processed 資料夾 export 成 MVDR 能認得的環境變數
        self.env = {**os.environ, "MVDR_FINAL": os.path.join(os.getcwd(), str(self.run_dir))}

        self.logger.info(f"MVDR setup 完成；repo={self.repo_dir}")
    ...
    def train(self, args):
        ...
        """
        print(self.run_dir)
        cmd = [f". common_settings/common.params.nq320k.sh && bash mvdr_run_scripts/run.nq.sh train {self.run_dir}"]       
    ...
    def evaluate(self, args):
        ...
        cmd = [f". common_settings/common.params.nq320k.sh && bash mvdr_run_scripts/run.nq.sh index {self.run_dir} {latest}"]
        ...
        # TODO: 依 MVDR 實際輸出格式萃取 MRR / Recall@K
        return metrics
    ...
    def _run_subprocess(self, cmd_list, cwd, stage: str) -> None:
        ...
        """
        self.logger.info(f"MVDR {stage} 指令：{' '.join(str(x) for x in cmd_list)}")
        subprocess.run(
            cmd_list,
            shell=True, 
            executable="/bin/bash",
        ...
        self.logger.info(f"MVDR {stage} 完成")
